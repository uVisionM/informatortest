...
import { AddFrontPage, Front } from '../atoms/AddFrontPage';
interface IFlipBook {
    pages: Array<
        | {
              clean: string;
...
export const FlipBook: React.FC<IFlipBook> = ({ pages, test, graduate, science }) => {
    useEffect(() => {
        ...
        for (let i = 0; i < pages.length; i++) {
            if (i >= 0) {
                AddPage('', 'flex flex-col ml-4 w-[95%]', '', pages[i]?.clean)
            }
        }
        AddFrontPage('', 'bg-pb w-full h-full flex flex-col py-[49%]', 'text-white text-4xl text-center', 'Nasi Absolwenci', '')
        graduate.map((g) => {
            AddPageImage('', 'flex flex-col ml-4 w-[95%]', 'mt-4', g!.clean, 'w-96 h-auto mx-auto mt-4', g!.changedToMatter.image);
        });
        AddFrontPage('', 'bg-pb w-full h-full flex flex-col py-[49%]', 'text-white text-4xl text-center', 'Koła naukowe na naszej uczelni!','')
        science.map((g) => {
            for (let i = 0; i < g.cleaned.length; i++) {
                AddPage('','flex flex-col ml-4 w-[95%]', 'flex justify-center', g.cleaned[i])
            }
        });
        pageFlip.on('init', () => {
